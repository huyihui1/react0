...
class PBAnalyzeList extends PureComponent {
  ...
  async componentDidMount() {
    ...
    const myChart = echarts.init(document.getElementById('miniTimeline'), 'light');
  ...
  initMap(myChart) {
    const option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          return `${moment(params[0].value[1]).format('MM-DD')}: ${params[0].value[0]}`;
        },
      },
      legend: {
        show: false,
  ...
  async fetchCtLabels(caseId) {
    const res = await ajax.get(`/cases/${caseId}/ct_labels?page=1&pagesize=${appConfig.largePageSize}`);
    console.log(res);
            ...
            ({ height, isScrolling, registerChild, onChildScroll, scrollTop }) => {
              return (
                <div id="windowScroller">
                  ...
                  <div className={toggleMiniTimeLine ? 'miniTimeline' : 'miniTimelineFixed'} style={{ visibility: Object.keys(criteria).length > 0 && !isLoading && !this.props.componentProps ? 'visible' : 'hidden', left: !toggleMiniTimeLine && iconOnly ? '-15px' : null, display: this.props.isLoading ? 'none' : 'block' }}>
                    <div id="miniTimeline" style={{ height: '100%' }} />
                    <div className="myTimeLine">
                      <div style={{ position: 'relative', width: '100%', height: '71%', top: '60px' }}>
                        <div className="line" />
                        {
                          caseEvents.items && caseEvents.items.filter(this.dateFilter).length > 0 ? (
                            caseEvents.items && caseEvents.items.filter(this.dateFilter).map((item, index) => {
                              if (moment(item.started_at).format('YYYY-MM-DD') > moment(summaryDate && summaryDate.endDate).format('YYYY-MM-DD')) {
                                if (caseEvents.items.length - 1 === index) {
                                  return <div className="endCircular noEvent" key={summaryDate.endDate + index} title={summaryDate && summaryDate.endDate} />;
                                }
                              } else {
                                ...
                                if (index === 0) {
                                  if (summaryDate && moment(item.started_at).format('YYYY-MM-DD') === summaryDate.startDate) {
                                    return (
                                      <Balloon align="t"
                                        closable={false}
                                        key={item.started_at + index}
                                        trigger={
                                          <div className="startCircular" style={{ background: item.color }} />
                                               }
                                        triggerType="hover"
                                      >
                                        <p>{summaryDate.startDate}</p>
                                        <span>{item.name}</span>
                                      </Balloon>
                                    );
                                  }
                                    if (caseEvents.items.length === 1) {
                                      return (
                                        <Fragment key={item.started_at + index}>
                                          <div className="startCircular noEvent" title={summaryDate && summaryDate.startDate} />
                                          <Balloon align="t"
                                            closable={false}
                                            key={item.started_at + index}
                                            trigger={
                                              <div className="startCircular" style={{ top: `${days / allDays * 100}%`, background: item.color }} />
                                                   }
                                            triggerType="hover"
                                          >
                                            <p>{item.started_at}</p>
                                          ...
                                          <div className="endCircular noEvent" title={summaryDate && summaryDate.endDate} />
                                        </Fragment>
                                      );
                                    }
                                      return (
                                ...
                                if (index > 0 && index < caseEvents.items.length - 1) {
                                  return (
                                    <Balloon align="t"
                                      ...
                                      triggerType="hover"
                                    >
                                      <p>{<item className="started_at" />}</p>
                                      <span>{item.name}</span>
                                    </Balloon>
                                  );
                                }
                                if (summaryDate && moment(item.started_at).format('YYYY-MM-DD') === summaryDate.endDate) {
                                  return (
                                    <Balloon align="t"
                                      closable={false}
                                      key={item.started_at + index}
                                      trigger={
                                        <div className="endCircular" style={{ background: item.color }} />
                                             }
                                      triggerType="hover"
                                    >
                                      <p>{item.started_at}</p>
                                ...
                                return (
                                  <Fragment key={item.started_at + index}>
                                    <Balloon align="t"
                                      ...
                                      key={item.started_at + index}
                                      trigger={
                                        <div className="endCircular" style={{ top: summaryDate && moment(item.started_at).format('YYYY-MM-DD') === summaryDate.startDate ? 0 : `${days / allDays * 100}%`, background: item.color }} />
                                             }
                                      triggerType="hover"
                                    >
                                      <p>{item.started_at}</p>
                        ...
                        {/* { */}
                        {/* caseEvents.items && caseEvents.items.length === 1 ? ( */}
                        {/* <div className="endCircular noEvent" title={summaryDate && summaryDate.endDate} /> */}
                        {/* ) : null */}
                        {/* } */}
                      </div>
