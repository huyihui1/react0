...
class FileImport extends Component {
  ...
  async onFileClick() {
    const res = await ajax.post(`/cases/${this.props.caseId}/rel_numbers/do-import`, {}, { withCredentials: true })
      .catch(err => {
  ...
  async onFileClose(bool = true) {
    if (this.state.file) {
      const res = await ajax.post(`/cases/${this.props.caseId}/rel_numbers/abort-import`, {}, { withCredentials: true })
        .catch(err => {
          Message.error(err.message);
        });
      if (res.meta.success) {
        ...
        >
          <IceContainer style={styles.container}>
            <Form>
              <FormItem>
                <Upload
                  ...
                  action={`${this.props.url}/cases/${this.props.caseId}/rel_numbers/upload`}
            ...
            <IceContainer style={{ padding: 0 }}>
              <span>数据模板:</span>
              <a style={{ marginLeft: '10px' }} href={`${ajax.baseUrl}/downloads/rel_numbers`}>亲情网模板.csv</a>
            </IceContainer>
            {
              this.state.isPreview ? (
                <IceContainer style={{ padding: 0, margin: '20px 0' }}>
                  ...
                  >
                    ...
                    <Table.Column align="center" title="亲情网名称" dataIndex="network" />
