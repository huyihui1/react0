...
class FilesTable extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.pbFileImps.meta && nextProps.pbFileImps.meta.page) {
      this.setState({
        current: nextProps.pbFileImps.meta.page.current,
        pageTotal: nextProps.pbFileImps.meta.page.total,
      });
    }
    if (nextProps.pbFileImps.items.length > 0) {
      this.setState({
        showButton: true
      })
    }
  ...
  render() {
    const { pbFileImps, uuid, isLoading } = this.props;
    return (
      <div>
        <Slider slidesToShow={pbFileImps.items[0] ? pbFileImps.items[0].length > 4 ? 4 : pbFileImps.items[0].length : 4} arrowPosition="outer" dots={false} >
          {
            this.formatData(pbFileImps.items[0]).map((item, index) => {
              return (
        ...
        </Slider>
        {
          this.state.showButton ? (
            <div style={{ textAlign: 'center', marginTop: '15px' }}>
              <Link to={`/cases/${this.state.case.id}/pb_analyze`}><Button type="primary">话单浏览</Button></Link>
            </div>
          ) : null
        }
      </div>
