...
let groupByBranchNumAndHourList = null;

export const {types, actions, rootReducer} = createResource({
  name: 'branchNumAndHour',
  url: `${hostUrl}/cases/:case_id/bbills/branch/group-by-branchnumandhour`,
  actions: {
    //B5-机构号vs交易时段(小时)
    getGroupBy: {
      method: 'POST',
      reduce: (state, action) => {
        ...
        if (action.body) {
          groupByBranchNumAndHourList = formatGroupByBranchNumAndHour(action.body.data);
        }
        if (status === 'resolved') {
    ...
    clear:{
      ...
      reduce: (state, action) => {
        return {
          ...state,
          groupByBranchNumAndHourList:null
        };
      },
    }
  },
