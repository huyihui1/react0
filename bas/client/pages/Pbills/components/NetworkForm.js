...
class NetworkForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      list: ['Lucy King', 'Lily King', 'Jim Green'],
      networkData: '',
      newsetData: {},
      tips: ''
    };
  ...
  onChangeShort(value) {
    ...
    // this.props.changeSetData(value, this.state.networkData.ownerNum)
  ...
  tableColumnRenderShort(value, index, record) {
    return (
      <div>
        ...
        {/*<div style={{color: 'red'}}>{this.state.tips}</div>*/}
      </div>
    );
  }
  ...
  tableColumnRenderNetwork(value, index, record) {
    return (
      <Select.AutoComplete dataSource={this.props.venNumBers} onChange={this.onChangeNetwork} defaultValue={value}/>
    );
  }
  ...
  validateAllFormField = () => {
    ...
    this.state.newsetData.forEach(item => {
      let obj = {
        num: item.owner_num,
        short_num: item.ven_short_num,
        network: item.ven_network
      };
      arr.push(obj)
    });

    const {actions, caseId} = this.props;
    actions.setNetworkPbills({caseId, values: arr}).then(res => {
  ...
  componentDidMount() {
    this.props.actions.getVenNumBersPbills({caseId: this.props.caseId});
  }
        ...
        >
          <div style={styles.formContent}>
            ...
            >
              ...
              <Table.Column title="短号" alignHeader="center" align='center' dataIndex="ven_short_num"
                            cell={this.tableColumnRenderShort}/>
              <Table.Column title="虚拟网名称" alignHeader="center" align='center' dataIndex='ven_network'
...
export default connect(
  ...
  state => ({
    ...
    venNumBers: state.pbills.venNumBers
  }),
  // mapDispatchToProps
  dispatch => ({
