...
(function(){
  ...
  var MarkerClusterer =
    ...
    BMapLib.MarkerClusterer = function(map, options){
      ...
      const mkrsOpt = opts['markersOpt'];
      if (mkrsOpt) {
        this._markersOpt = mkrsOpt;
      }
  ...
  MarkerClusterer.prototype._createClusters = function(){
    ...
    for(var i = 0, marker; marker = this._markers[i]; i++){
      if(!marker.isInCluster && extendedBounds.containsPoint(marker.getPosition()) ){
        this._addToClosestCluster(marker, this._markersOpt[i]);
      }
    }
   ...
   * @return 无返回值。
   */
  MarkerClusterer.prototype._addToClosestCluster = function (marker, markerOpt){
    ...
    if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)){
      clusterToAddTo.addMarker(marker, markerOpt);
    } else {
      var cluster = new Cluster(this);
      cluster.addMarker(marker, markerOpt);
  ...
  function Cluster(markerClusterer){
    ...
    this._markersOpt = [];//这个Cluster中所包含的markers的配置属性
   ...
   * @return 无返回值。
   */
  Cluster.prototype.addMarker = function(marker, markerOpt){
    ...
    this._markersOpt.push(markerOpt)
  };
   ...
   * @return 无返回值
   */
  Cluster.prototype.render = function(index){
    var len = this._markers.length;
    ...
    if (len < this._minClusterSize) {
      ...
      this._map.addOverlay(this._clusterMarker);
      this._isReal = true;
      this.updateClusterMarker(index);
    }
  }
   ...
   * @return 无返回值。
   */
  Cluster.prototype.updateClusterMarker = function (index) {
    ...
    let count = 0;
    let li = '';
    this._markersOpt.forEach(item => {
      count += item.count * 1;
      li += `<li style="font-size: 13px;"><span style="margin-right: 10px">${item.ct_code}:</span> <span>${item.count}</span></li>`;
    });
    const liElement = `<div style="max-width: 600px">
                              <h3><span style="margin-right: 10px">基站数:${this._markers.length}</span><span>通话记录总计: ${count}</span></h3>
                              <ul style="max-height: 100px; overflow-y: auto;">
                                ${li}
                              </ul>
                       </div>`;
    this._clusterMarker.setText(count);
    // console.log(this._markerClusterer._markersOpt);
    var thatMap = this._map;
    var that = this;
    var thatBounds = this.getBounds();
    this._clusterMarker.addEventListener("click", function(event){
      const infoWindow = new window.BMap.InfoWindow(liElement, {
        offset: new window.BMap.Size(0, -15),
      });
      thatMap.openInfoWindow(infoWindow, that._center);
