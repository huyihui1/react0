...
import {actions, formatCodeandstartedtimel2class} from '../../../stores/pbStat/codeAndstartedtimel2Class';
...
class CodeAndstartedtimel2class extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isChart: false,
      ...
      hotSetting: {
        ...
        columns: columns.codeAndstartedtimel2class,
      ...
      drilldownOptions: {
        '总计': ['owner_ct_code'],
        '04:30 ~ 06:20': ['owner_ct_code',{started_time_l2_class:'0'}],
        '06:21 ~ 07:10': ['owner_ct_code',{started_time_l2_class:'1'}],
        '07:11 ~ 07:50': ['owner_ct_code',{started_time_l2_class:'2'}],
        '07:51 ~ 08:25': ['owner_ct_code',{started_time_l2_class:'3'}],
        '08:26 ~ 11:00': ['owner_ct_code',{started_time_l2_class:'4'}],
        '11:01 ~ 11:30': ['owner_ct_code',{started_time_l2_class:'5'}],
        '11:31 ~ 12:30': ['owner_ct_code',{started_time_l2_class:'6'}],
        '12:31 ~ 13:20': ['owner_ct_code',{started_time_l2_class:'7'}],
        '13:21 ~ 14:00': ['owner_ct_code',{started_time_l2_class:'8'}],
        '14:01 ~ 16:50': ['owner_ct_code',{started_time_l2_class:'9'}],
        '16:51 ~ 17:40': ['owner_ct_code',{started_time_l2_class:'10'}],
        '17:41 ~ 18:50': ['owner_ct_code',{started_time_l2_class:'11'}],
        '18:51 ~ 20:00': ['owner_ct_code',{started_time_l2_class:'12'}],
        '20:01 ~ 21:50': ['owner_ct_code',{started_time_l2_class:'13'}],
        '21:51 ~ 23:59': ['owner_ct_code',{started_time_l2_class:'14'}],
        '00:00 ~ 04:29': ['owner_ct_code',{started_time_l2_class:'15'}],
      },
      points: [],
      hotMapData: [],
      showHeatmap: true,
  ...
  componentDidMount() {
    // const myChart = echarts.init(document.getElementById('codeandstartedtimel2class'), 'light');
    // this.initC3(myChart);
    if (window.BMap) {
      this.renderMap();
    } else {
  ...
  renderMap(x = 116.404, y = 39.915) {
    ...
    this.map = new window.BMap.Map('codeandstartedtimel2class', {enableMapClick: false});
  ...
  onSelectChange = async (value) => {
    ...
    let res = await ajaxs.post(`/cases/${this.props.caseId}/pbills/overview/group-by-codeandstartedtimel2class`, {criteria: data});

    if (res.meta.success) {
      let p = formatCodeandstartedtimel2class(res.data).points;
      this.renderHotMap(p);
  ...
  onChangeDouble = (doubleValue) => {
    ...
    ajaxs.post(`/cases/${this.props.caseId}/pbills/overview/group-by-codeandstartedtimel2class`, {
      criteria,
      view: {}
    }).then(res => {
  ...
  getImgURL() {
    downloadIamge(this.state.myChart, 'B3-基站vs通话时段(详细)');
  }

  getExcel() {
    ajaxs.post(`/cases/${this.props.caseId}/pbills/overview/group-by-codeandstartedtimel2class.xlsx`, {
  ...
  fetchData(params) {
    const {fetchCodeandstartedtimel2class} = this.props.actions;
    fetchCodeandstartedtimel2class({case_id: this.props.caseId, ...{criteria: params, view: {}}});
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.codeandstartedtimel2classHotMap && JSON.stringify(nextProps.codeandstartedtimel2classHotMap) !== JSON.stringify(this.state.hotMapData)) {
      firstReq = true;
      this.renderHotMap(nextProps.codeandstartedtimel2classHotMap);
      this.setState({
        points: nextProps.codeandstartedtimel2classHotMap,
        hotMapData: nextProps.codeandstartedtimel2classHotMap
      });
    }

    if (nextProps.codeandstartedtimel2classList && nextProps.codeandstartedtimel2classList !== this.state.codeandstartedtimel2classList) {
      this.setState({
        codeandstartedtimel2classList: nextProps.codeandstartedtimel2classList
      })
    }
  ...
  componentWillUnmount() {
    this.props.actions.clearCodeandstartedtimel2class();
    this.unmountCompsOnDoms();
  }
  ...
  render() {
    return (
      <div className="chart-item">
        <ChartTitle title="B3-基站vs通话时段(详细)" align={'center'} handleChart={this.handleChart} getImgURL={this.getImgURL}
                    getExcel={this.getExcel}/>
        {/*<div id="codeandstartedtimel2class" style={{height: 300, display: this.state.isChart ? 'block' : 'none'}}/>*/}
        {/*{*/}
        {/*!this.state.isChart ?*/}
        {/*<ExcelView id="codeandstartedtimel2classExcel" hotSetting={this.state.hotSetting || null}*/}
        {/*colHeaders={this.state.colHeaders}*/}
        {/*data={this.props.codeandstartedtimel2classList}/> : null*/}
        {/*}*/}
        <ExcelView id="codeandstartedtimel2classExcel" hotSetting={this.state.hotSetting || null}
                   colHeaders={this.state.colHeaders}
                   drilldown={this.state.drilldownOptions}
                   data={this.state.codeandstartedtimel2classList}
        ...
        <div id="codeandstartedtimel2class" className={'pbStatChart'}/>
      </div>
    );
  }
}
...
export default connect(
  ...
  state => ({
    ...
    codeandstartedtimel2classList: state.codeAndstartedtimel2Class.codeandstartedtimel2classList,
    codeandstartedtimel2classItems: state.codeAndstartedtimel2Class.codeandstartedtimel2classItems,
    codeandstartedtimel2classHotMap: state.codeAndstartedtimel2Class.codeandstartedtimel2classHotMap,
    isLoading: state.codeAndstartedtimel2Class.isLoading,
    search: state.search,
