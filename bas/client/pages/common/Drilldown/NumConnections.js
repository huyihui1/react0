...
import BatchMapComponent from '../LocGroupMap';
...
import {
  ...
  handleLoading,
  searchStartedday,
  searchNumConnections
} from '../../../stores/simplePbillRecordList/actions';
...
class NumConnections extends PureComponent {
      ...
      if ((dataKey === 'owner_ct_code' && ownerCodeArr.indexOf(rowIndex) !== -1) || (dataKey === 'peer_ct_code' && peerCodeArr.indexOf(rowIndex) !== -1)) {
        if (isLabel) {
          return (
            <Fragment>
              <span
                style={{display: 'inline-block', width: '100%', backgroundColor: isLabel.marker_color, color: '#fff'}}
                labeldata={JSON.stringify({code: cellData})} onClick={(e) => {
                this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
              }}>{isLabel.label}</span>
            </Fragment>
          );
        }
        return (
          <HighlightCode cellData={cellData} dataKey={dataKey} rowData={rowData} rowIndex={rowIndex}
                         clickHandler={this.clickHandler} codeColors={this.codeColors}
                         extType={'SimplePbillRecordList'}/>
        );
      }
      return (
        <span className='ctLocText' labeldata={JSON.stringify({code: cellData})} onClick={(e) => {
          this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
        }}
              title={dataKey === 'peer_ct_code' ? rowData.peer_ct_dist + rowData.peer_ct_town || noDataText : rowData.owner_ct_dist + rowData.owner_ct_town || noDataText}>{dataKey === 'peer_ct_code' ? rowData.peer_ct_dist + rowData.peer_ct_town || noDataText : rowData.owner_ct_dist + rowData.owner_ct_town || noDataText}</span>
      );
    }
    return null;
  }
  ...
  cellRenderer({cellData, dataKey, rowData, rowIndex}) {
    ...
    if (dataKey === 'owner_num' || dataKey === 'peer_num') {
      ...
      return (
        <div className="num-box" style={{display: 'inline-flex'}} onClick={(e) => {
          this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
        }}>
          <span>
            {cellData}
          ...
          return (
            <div title={result.ownerCname.item.label}>
              <IceLabel inverse={false} style={{
                fontSize: '12px',
                backgroundColor: result.ownerCname.item.label_bg_color,
                color: result.ownerCname.item.label_txt_color,
                padding: '2px'
              }}>{result.ownerCname.item.label}</IceLabel>
            </div>
          );
        }
        if (result.peerCname && result.peerCname.key === dataKey) {
          return (
            <div title={result.peerCname.item.label}>
              <IceLabel inverse={false} style={{
                fontSize: '12px',
                backgroundColor: result.peerCname.item.label_bg_color,
                color: result.peerCname.item.label_txt_color,
                padding: '2px'
              }}>{result.peerCname.item.label}</IceLabel>
            </div>
          );
        }
      } else {
    ...
    if (dataKey === 'started_day') {
                    ...
                    <li key={item.name + index}>
                      <h3>
                        <IceLabel inverse={false} style={{
                          ...
                          color: '#fff',
                          padding: '2px'
                        }}>{moment(item.started_at).format('YYYY-MM-DD')}{item.ended_at ? `~${moment(item.ended_at).format('YYYY-MM-DD')}` : null}</IceLabel>
      ...
      if (rowIndex === 0) {
        ...
        if (res.length > 0) {
          return (
            <Balloon align="l"
                     closable={false}
                     trigger={
                       <div style={{color: 'blue', fontWeight: 'bold'}} onClick={(e) => {
                         this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
                       }}>
                         <div style={{display: 'inline-block', position: 'relative'}}
                              labeldata={`${JSON.stringify({time: `${rowData.started_day} ${rowData.started_time}`})}`}>
                           {cellData}
                           <div style={{
                             display: 'flex',
                             flexDirection: 'row',
        ...
        return <div style={{color: 'blue', fontWeight: 'bold'}}
                    labeldata={`${JSON.stringify({time: `${rowData.started_day} ${rowData.started_time}`})}`}
                    onClick={(e) => {
                      this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
                    }}>{cellData}</div>;
      ...
      if (this.startedDay !== cellData) {
        ...
        if (res.length > 0) {
          return (
            <Balloon align="l"
                     ...
                     trigger={
                       <div style={{color: 'blue', fontWeight: 'bold'}} onClick={(e) => {
                         this.clickHandler(e, rowData, cellData, dataKey);
                       }}>
        ...
        return <div style={{color: 'blue', fontWeight: 'bold'}}
                    labeldata={`${JSON.stringify({time: `${rowData.started_day} ${rowData.started_time}`})}`}
                    title={`${solar2lunarData.monthCn}${solar2lunarData.dayCn}`}>{cellData}</div>;
      } else if (this.startedDay === cellData) {
        return <div style={{color: '#bdb5b5'}} title={`${solar2lunarData.monthCn}${solar2lunarData.dayCn}`}
                    labeldata={`${JSON.stringify({time: `${rowData.started_day} ${rowData.started_time}`})}`}
                    onClick={(e) => {
                      this.clickHandler(e, rowData, cellData, dataKey);
                    }}>{moment(cellData).format('YYMM.DD日')}</div>;
      }
    }
    if (dataKey === 'started_time' && cellData) {
      return <span
        labeldata={`${JSON.stringify({time: `${rowData.started_day} ${rowData.started_time}`})}`}>{cellData}</span>;
    }
      ...
      if (rowData.owner_ct_code && rowData.peer_ct_code) {
        return (
          <Balloon align="rb"
                   ...
                   closable={false}
                   trigger={
                     <div style={{color: cellData === '主叫' ? '#f63' : null}} onClick={(e) => {
                       this.clickHandler(e, rowData, cellData, dataKey);
                     }}>{cellData}</div>
                   }
          ...
          >
            <div style={{width: '600px', height: '350px'}}>
              <MapComponent code={[rowData.owner_ct_code, rowData.peer_ct_code]} ctLabel={this.state.ctLabel}
                            commDirection={rowData.comm_direction} cellData={cellData}/>
            </div>
          </Balloon>
        );
      }
      return <div style={{color: cellData === '主叫' ? '#f63' : null}} onClick={(e) => {
        this.clickHandler(e, rowData, cellData, dataKey);
      }}>{cellData}</div>;
    }
    ...
    if (this.loc && this.attr) {
      if (this.attr.indexOf(this.loc) === -1 && dataKey === 'peer_comm_loc') {
        return <div style={{color: '#f63'}} onClick={(e) => {
          this.clickHandler(e, rowData, cellData, dataKey);
        }}>{this.loc}</div>;
      }
    }
    if (dataKey === 'started_time_l1_class') {
      if (cellData === '傍晚' || cellData === '晚上') {
        return <div style={{color: 'blue'}} onClick={(e) => {
          this.clickHandler(e, rowData, cellData, dataKey);
        }}>{cellData}</div>;
      }
      if (cellData === '深夜' || cellData === '凌晨') {
        return <div style={{color: '#f33'}} onClick={(e) => {
          this.clickHandler(e, rowData, cellData, dataKey);
        }}>{cellData}</div>;
      }
    }
              ...
              <Balloon animation={false}
                       ...
                       trigger={<span style={{display: 'inline-block'}}>
                  {
                    this.codeLocRender(showCtLoc, isLabel, ownerCodeArr, peerCodeArr, rowIndex, cellData, dataKey, rowData)
                           ...
                           showCtLoc === false ? (((dataKey === 'owner_ct_code' && ownerCodeArr.indexOf(rowIndex) !== -1) || (dataKey === 'peer_ct_code' && peerCodeArr.indexOf(rowIndex) !== -1)) ? (
                             ...
                             <Fragment>
                               <span style={{
                                 ...
                                 width: '100%',
                                 backgroundColor: isLabel.marker_color,
                                 color: '#fff'
                               }} labeldata={JSON.stringify({code: cellData})} onClick={(e) => {
                                 this.clickHandler(e, rowData, cellData, dataKey, rowIndex);
                               }}>{isLabel.label}</span>
                       ...
                       triggerType="hover"
              >
                <BatchMapComponent code={codes} id={codes + rowIndex} activeCode={cellData} ctLabel={this.state.ctLabel}
          ...
          <Balloon animation={false}
                   ...
                   trigger={<span style={{display: 'inline-block'}}>
              {
                this.codeLocRender(showCtLoc, isLabel, ownerCodeArr, peerCodeArr, rowIndex, cellData, dataKey, rowData)
                   ...
                   triggerType="hover"
                   onVisibleChange={(b) => {
                     this.onVisibleChange(b, cellData);
                   }}
          >
            <div style={{width: '100%'}}>
              {
                isLabel ? (
                  <h3 style={{marginBottom: '5px'}}>
                    ...
                    color: '#fff',
                    padding: '2px'
                  }}>{isLabel.label}</IceLabel>
                  </h3>
                ) : null
              }
              <p style={{fontSize: '14px', marginTop: '0'}}>
                <span style={{fontSize: '16px'}}>基站:</span> {cellData} <FontAwesomeIcon icon={faExchangeAlt}
                                                                                        style={{marginLeft: '8px'}}/> {`${parseInt(cellData.split(':')[0], 16)}:${parseInt(cellData.split(':')[1], 16)}:${parseInt(cellData.split(':')[2], 16)}`}
              </p>
              <p style={{
                minWidth: '240px',
                fontSize: '14px'
              ...
              <div id="minMap2" style={{width: '100%', height: '300px'}}/>
            </div>
          </Balloon>
        );
      }
  ...
  onScroll({scrollTop}) {
    ...
    if ((scrollTop > bodyScrollHeight / 2) && this.state.isRequest) {
      let params = {...this.props.criteria, last_alyz_day: this.props.PBAnalyzes.last_alyz_day};
      if (this.props.pagingType) {
        params = this.props.pagingType(this.props.PBAnalyze, this.state.componentData);
      }
      this.props.searchPBAnalyze(this.props.caseId, params, false, `/cases/${this.props.caseId}/pbills/records/search`, true).then(res => {
  ...
  async componentDidMount() {
    const starteddayChart = echarts.init(document.getElementById("numConnections"), 'light');
    starteddayChart.showLoading({text: appConfig.LOADING_TEXT});
    if (this.props.componentProps && this.props.componentProps.opt && JSON.stringify(this.props.componentProps.opt) !== JSON.stringify(this.state.componentPropsOpt)) {
      this.props.searchNumConnections(this.props.caseId, {...this.props.componentProps.opt}, this.props.componentProps && this.props.componentProps.url).then(res => {
        if (res) {
          starteddayChart.hideLoading();
          this.initC3(starteddayChart,res,JSON.parse(JSON.stringify(this.props.componentProps)));
        }
      });
      this.flag = true;
      this.setState({
  ...
  initC3(starteddayChart,data,params) {
    this.setState({
      starteddayChart,
    });
    let xData = [];
      ...
      series: [
        {
          ...
          itemStyle: {
            normal: {
              color: data => {
                let itemColor = '#3398DB';
                if (data.dataIndex === this.state.dataIndex){
                  itemColor = 'red'
                } else {
                  itemColor = '#3398DB'
                }
    ...
    window.addEventListener('resize', () => {
      starteddayChart.resize();
    });

    let that = this;
    // delete params.opt.criteria.bill_type;
    params.opt.criteria.owner_num = ['IN',xData[0]];
    this.props.searchPBAnalyze(this.props.caseId, { ...params.opt }, true,`/cases/${this.props.caseId}/pbills/records/search`);
    this.setState({componentData:params});

    starteddayChart.setOption(option);
    starteddayChart.on('click',function (e) {
      params.opt.adhoc.page = 1;
      params.opt.criteria.owner_num = ['IN',e.name];
      that.props.searchPBAnalyze(that.props.caseId, { ...params.opt }, true,`/cases/${that.props.caseId}/pbills/records/search`);
      that.setState({isRequest:true,dataIndex:e.dataIndex,componentData:params});
      that.props.clearPage();
      // starteddayChart.setOption(option);
      starteddayChart.setOption({
        dataZoom:[{
          start:start,
          end:end,
        }]
    ...
    starteddayChart.on('datazoom', function (e) {
      if (e.batch) {
  ...
  async componentWillReceiveProps(nextProps) {
    if (!this.flag && nextProps.componentProps && nextProps.componentProps.opt && JSON.stringify(nextProps.componentProps.opt) !== JSON.stringify(this.state.componentPropsOpt)) {
      this.props.searchPBAnalyze(nextProps.caseId, {...nextProps.componentProps.opt}, true, nextProps.componentProps && nextProps.componentProps.url);
      this.setState({
    ...
    if (nextProps.criteria && Object.keys(nextProps.criteria).length !== 0 && this.state.criteria !== nextProps.criteria) {
      ...
      console.log("============", nextProps.componentProps, nextProps.componentProps.opt);
      if (nextProps.componentProps && nextProps.componentProps.opt) {

      } else {
        this.props.searchPBAnalyze(nextProps.caseId, {...nextProps.criteria}, true, nextProps.componentProps && nextProps.componentProps.url);
      }
  ...
  render() {
    ...
    const params = {
      criteria: countMessage ? countMessage(this.state.criteria, rowData) : {...this.props.criteria},
      hlColField,
      hlColValues,
      colValues
    };
    return (
      <Fragment>
        <div style={{display: showTableMessage ? 'block' : 'none'}}>
          <PBTableMessage data={params} caseId={caseId} onSubmit={this.offSelectData}
                          fetchCountByDist={this.fetchCountByDist}/>
        </div>
        <div id="windowScrollerTable2">
          <div id="numConnections" style={{height: '350px', width: '100%'}}/>
          <AutoSizer disableHeight>
            {
              ({width}) => {
                return (
                  <div>
                    {/*<div  onContextMenu={this.handleEvent}>*/}
                    {/*<MenuProvider id="menu_id" event="handleEvent">*/}
                    <div className={this.state.isScrollTitle ? 'p-fixed' : 'p-absolute'}
                         style={{top: '390px', right: '15px'}}>
...
const mapStateToProps = (state) => {
  return {
    ...
    PBAnalyzes: state.SimplePbillRecordList,
    caseId: state.cases.case.id,
    isLoading: state.SimplePbillRecordList.isLoading,
    iconOnly: state.cases.iconOnly,
    ext: state.SimplePbillRecordList.ext,
    search: state.search,
  };
};
...
const withReducer = injectReducer({key: 'SimplePbillRecordList', reducer: PBAnalyzeReducer});
