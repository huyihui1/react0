...
class SearchBar extends Component {
  ...
  validateFields = (bool = true) => {
    ...
    validateAll((errors, values) => {
      ...
      if (val.radius && val.loc_rule === 'scope_ct') {
        data.adhoc.radius = parseFloat(val.radius);
      }
      if (val.loc_rule) {
        data.adhoc.loc_rule = val.loc_rule;
        setLocRuleSearch(val.loc_rule);
      }
      if (val.mutual_call !== undefined) {
        data.adhoc.mutual_call = val.mutual_call * 1;
      }
      if (val.alyz_day) {
        let newArr = [];
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.search.mySearchs && JSON.stringify(nextProps.search.mySearchs) !== JSON.stringify(this.state.initMySearchs)) {
      let mySearchs = formatAdvancedSearch(nextProps.search.mySearchs, 'meets');
      this.setState({
        mySearchs,
                ...
                >
                  <div style={styles.container2}>
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        <span style={{ ...styles.caseNumber, width: 'initial', marginRight: 0 }}>
                          <label style={styles.label}>
                            <span style={{ whiteSpace: 'nowrap' }}>是否通话:</span>
