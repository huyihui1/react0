...
class BBAnalyzeList extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      rowIndex: -2,
      ...
      case 'owner_card_num':
        ...
        return (
          <Balloon align="t"
                   ...
                   trigger={
                     <div>
                       <BankCardSimple card={ownerVal} rowData={rowData} num={rowData.owner_card_num} acct={rowData.owner_bank_acct} card_type={rowData.owner_card_type} bankCode={rowData.owner_bank_code} />
                     </div>
                   }
          >
            <BankCardFull
        ...
        return (
          <Balloon align="t"
            ...
            // triggerType="click"
                   trigger={
                     <div>
                       <BankCardSimple card={val} rowData={rowData} num={rowData.peer_card_num} acct={rowData.peer_bank_acct} card_type={rowData.peer_card_type} bankCode={rowData.peer_bank_code} />
                     </div>
                   }
          >
            <BankCardFull
  ...
  componentDidMount() {
    ...
    window.onscroll = () => {
      ...
      if (Array.isArray(this.props.bbAnalyzes.items) && this.props.bbAnalyzes.items.length == 0) return;
  ...
  onScroll = ({ scrollTop }) => {
    ...
    if (scrollTop > 0 && this.props.bbAnalyzes.items.length > 0) {
      this.setState({
        isScrollTitle: true,
      });
      this.props.isScrollTitle(true)
    } else {
      this.setState({
        isScrollTitle: false,
      });
      this.props.isScrollTitle(false)
    }
    if (this.props.noPaging) {
      return;
    }
    if ((scrollTop > bodyScrollHeight / 2) && this.state.isRequest && this.state.params) {
      const {actions, caseId, search} = this.props;
      let params = { ...search.params };
      params.adhoc.page = this.page;
                        ...
                        <div ref={registerChild} onContextMenu={this.handleEvent}>
                          <Loading visible={this.state.isLoading} style={{ height: 400 }} tip="加载中...">
                            <Table
                              ...
                              headerHeight={70}
                              rowHeight={50}
                              rowCount={bbAnalyzes.items.length}
                              rowGetter={({ index }) => bbAnalyzes.items[index]}
