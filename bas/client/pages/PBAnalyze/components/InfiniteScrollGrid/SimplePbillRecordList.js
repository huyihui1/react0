...
import { getPBAnalyze, clearItems, searchPBAnalyze, searchCode, handleLoading } from '../../../../stores/simplePbillRecordList/actions';
...
class SimplePbillRecordList extends PureComponent {
  ...
  codeLocRender = (showCtLoc, isLabel, ownerCodeArr, peerCodeArr, rowIndex, cellData, dataKey, rowData) => {
    if (showCtLoc) {
      if ((dataKey === 'owner_ct_code' && ownerCodeArr.indexOf(rowIndex) !== -1) || (dataKey === 'peer_ct_code' && peerCodeArr.indexOf(rowIndex) !== -1)) {
        ...
        return (
          <HighlightCode cellData={cellData} dataKey={dataKey} rowData={rowData} rowIndex={rowIndex} clickHandler={this.clickHandler} codeColors={this.codeColors} extType={'SimplePbillRecordList'} />
        );
      }
      return (
  ...
  onScroll({ scrollTop }) {
    ...
    if ((scrollTop > bodyScrollHeight / 2) && this.state.isRequest) {
      ...
      if (this.props.pagingType) {
        params = this.props.pagingType(this.props.PBAnalyze, this.props.componentProps);
      }
  ...
  async componentDidMount() {
    if (this.props.componentProps && this.props.componentProps.opt && JSON.stringify(this.props.componentProps.opt) !== JSON.stringify(this.state.componentPropsOpt)) {
      this.props.searchPBAnalyze(this.props.caseId, { ...this.props.componentProps.opt }, true, this.props.componentProps && this.props.componentProps.url);
      this.flag = true;
      this.setState({
  ...
  render() {
    ...
    const { showTableMessage, hlColField, hlColValues, colValues, criteria, toggleMiniTimeLine, summaryDate, rowData,scrollToIndex } = this.state;
    let allDays = null;
    if (summaryDate) {
                ...
                return (
                  ...
                  {/*<div  onContextMenu={this.handleEvent}>*/}
                    {/*<MenuProvider id="menu_id" event="handleEvent">*/}
                      <div className={this.state.isScrollTitle ? 'p-fixed' : 'p-absolute'} style={{top: '45px', right: '15px'}}>
