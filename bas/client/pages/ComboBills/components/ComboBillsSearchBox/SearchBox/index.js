...
import { Input, Select, DatePicker, Button, Message, Dialog, Tag, Icon, Balloon } from '@alifd/next';
...
import { actions as comboBillsActions } from '../../../../../stores/comboBills/index';
...
import BbSearchBox from './BbSearchBox';
...
class SearchBox extends Component {
  ...
  async fetchData() {
    ...
    const { getOwnerNumSearch, fetchMySearchSearch} = this.props.actions;
    getOwnerNumSearch({ case_id: caseId });
    fetchMySearchSearch({subject: 'pbill', case_id: caseId });
  }
  ...
  validateFields = (bool = true) => {
    ...
    validateAll((errors, values) => {
      ...
      if (bool) {
        ...
        // clearPBStatStoreChart();
        // setCriteriaSearch({ ...val });
        if (!this.props.isHide && this.props.search.chartView.length === 0) {
  ...
  saveSearch() {
    if (this.state.searchName) {
      const values = formatFormData(this.validateFields(false), true);
      ...
      const res = { criteria: { ...values }, view: { 'order-by': orderBy }, adhoc: { daily_rec } };
      const params = {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.search.mySearchs && JSON.stringify(nextProps.search.mySearchs) !== JSON.stringify(this.state.initMySearchs)) {
      const mySearchs = formatAdvancedSearch(nextProps.search.mySearchs, this.props.isHide ? 'pbAnalyze' : 'pbStat');
      this.setState({
        mySearchs,
  ...
  onSearch = () => {
    ...
    const pbill = this.getPbFormData();
    const bbill = { criteria: this.BbSearch.validateFields().criteria };
    actions.setSearchComboBills({ pbill, bbill });
  };
  ...
  render() {
    ...
    const showAdvancedCondition = this.state.conditions.map((cond, index) => {
      return (
        <Message key={cond[0]} shape="toast"
    ...
    const showBBAdvancedCondition = this.state.bbConditions.map((cond, index) => {
      return (
          ...
          <IceContainer style={styles.container}>
            <div style={styles.row}>
                        ...
                        <label style={styles.label}>
                          ...
                          <FormBinder name="owner_num">
                            ...
                            <NumberSelector name="owner_num"
                                            labelGroup={this.props.labelGroup}
                      ...
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <span>
                          <Balloon trigger={<Button>
                            我的筛选模板
                          </Button>} align="t" alignEdge style={{ width: 500 }}>
                            {/* 我的搜索 */}
              ...
              <div style={styles.right}>
                <BbSearchBox isShow onRef={this.onRef} onChangeBBForm={this.onChangeBBForm} />
              </div>
            </div>
            {showAdvancedCondition}
            {showBBAdvancedCondition}
...
export default connect(
  ...
  dispatch => ({
    actions: bindActionCreators({
      ...actions, ...pbStatActions, ...labelPNActions, ...labelCellActions, ...comboBillsActions,
      clearItems,
    }, dispatch),
  }),
)(SearchBox);
