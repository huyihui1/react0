...
import { formatFormData, renameLabel } from '../../../utils/utils';
...
class SearchBar extends Component {
  ...
  async fetchData() {
    ...
    const { getOwnerNumSearch, fetchLabelCells, fetchMySearchSearch, fetchLabelPNs } = this.props.actions;
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.search.mySearchs && JSON.stringify(nextProps.search.mySearchs) !== JSON.stringify(this.state.initMySearchs)) {
      let mySearchs = formatAdvancedSearch(nextProps.search.mySearchs, 'mutual');
      this.setState({
        mySearchs,
      ...
      <div>
        <div className={this.state.isShow ? 'PBSearch show' : 'PBSearch'}>
          {/*<div style={{ ...styles.nav, marginBottom: '10px' }}>*/}
            ...
            {/*</div>*/}
          {/*</div>*/}
          <PageTitle title={this.props.title} tour={{page: "mutuai"}}/>
                       ...
                       <span style={styles.caseNumber}>
                        <label style={styles.label}>
                          ...
                          <FormBinder name="owner_num">
                            {/* 自定义封装号码选择器 */}
                            <SingleNumberSelector name="owner_num" hideLabel={true} labelGroup={this.props.labelGroup} dataSource={search.ownerNums} values={this.state.values} onFormChange={this.onFormChange} isLabel={this.state.labelPNs} />
                          </FormBinder>
                        </label>
                      </span>
                      <br />
                      <span style={styles.caseNumber}>
                        <label style={styles.label}>
                          ...
                          <FormBinder name="peer_num">
                            <SingleNumberSelector name="peer_num" hideLabel={true} dataSource={search.ownerNums} values={this.state.values} onFormChange={this.onFormChange} isLabel={this.state.labelPNs} />
                          </FormBinder>
                        </label>
                      </span>
                      <br />
        ...
        </div>
        {
          this.state.isShow ? null : this.state.conditions.length > 0 ? (
            ...
            <PageTitle title={this.props.title} tour={{page: "mutuai"}} collapsed={true}/>
          )
        }
