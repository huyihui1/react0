...
import { getAllDate, getMonthAll, getQuarterAll, getWeeks } from '../../../../../utils/bbillsUtils';
import appConfig from '../../../../../appConfig';
import { dayMissingAsZero, monthMissingAsZero, quarterMissingAsZero, yearMissingAsZero, weekMissingAsZero, tenDaysMissingAsZero } from '../../../../../utils/timeframe';
...
class TimelyNet extends Component {
  constructor(props) {
    super(props);
    this.state = {
      myChart: null,
  ...
  componentDidMount() {
    const chart = document.querySelector('#tendencyBox');
    chart.style.height = `${document.documentElement.offsetHeight}px`;
    const myChart = echarts.init(document.getElementById('tendency'), 'light');
  ...
  tooltipReder = (params) => {
    return `
      <div>
        ...
        <div>
          <span>${params[2].marker}</span><span>${params[2].seriesName}:</span>
          <span>${formatMoney(params[2].value)}</span>
        </div>
      </div>
    `
  }
  ...
  fetchData = (params) => {
      ...
      yAxis: {
        ...
        max: value => {
          if (!this.max && value.max > 0) {
            ...
            this.max = value.max;
          } else if (this.max < value.min * -1) {
            this.max = value.min * -1;
          }
    ...
    ajaxs.post(`/cases/${this.props.caseId}/balance/timely-net`, params).then(res => {
      if (res.meta.success) {
        ...
        if (params.adhoc && params.adhoc['group-by'] === 'trx_day') {
          const result = dayMissingAsZero(res.data, { start: startDate, end: endDate }, {
            key: 'trx_day',
            value: 'cash_net',
          });
          ...
          const result = monthMissingAsZero(res.data, { start: startDate, end: endDate }, {
            key: ['year', 'month'],
            value: 'cash_net',
          });
          dateArr = result.dateArr;
          ...
          let result = quarterMissingAsZero(res.data, {start: startDate, end: endDate}, {
            key: ['year', 'quarter'],
            value: 'cash_net'
          });
          dateArr = result.dateArr;
          ...
          let result = weekMissingAsZero(res.data, {start: startDate, end: endDate}, {
            key: ['year', 'week'],
            value: 'cash_net'
          })
          dateArr = result.dateArr;
          ...
          let result = yearMissingAsZero(res.data, {start: startDate, end: endDate}, {
            key: 'year',
            value: 'cash_net'
          });
          dateArr = result.dateArr;
          ...
          let result = tenDaysMissingAsZero(res.data, {start: startDate, end: endDate}, {
          ...
          series: [
            ...
            },
            {
              ...
              data: JSON.parse(JSON.stringify(data)).map(item => {
                return item.value = item.cash_in || 0;
              }),
            },
            {
              type: 'bar',
              ...
              stack: 'one',
              data: JSON.parse(JSON.stringify(data)).map(item => {
                return item.value = -item.cash_out || 0;
              }),
            },
          ],
        }, false);
  ...
  render() {
    return (
      <div id="tendencyBox" style={styles.container}>
        <div id="tendency" style={{ width: '100%', height: '100%' }} />
        <div style={{
          position: 'absolute',
          top: 0,
          width: '100%',
