...
class DigestChart extends Component {
  constructor(props) {
    super(props);
    this.state = {
      myChart: null,
  ...
  componentDidMount() {
    const myChart = echarts.init(document.getElementById('digest'), 'light');
  ...
  fetchData(id) {
    ...
    ajaxs.post(`/cases/${id}/bbills/overview/group-by-digest`, {...params}).then(res => {
      ...
      <div style={{ height: '200px' }}>
        {
          this.state.dailyCallArr && this.state.dailyCallArr.length === 0 ? (
            ...
            <div id="digest" style={{ height: '100%' }} />
          )
        }
      </div>
    );
