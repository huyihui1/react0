...
let tellerandtimel1List = null;

export const {types, actions, rootReducer} = createResource({
  name: 'tellerandtimel1',
  url: `${hostUrl}/cases/:case_id/bbills/branch/group-by-tellerandtimel1`,
  actions: {
    //B3-机构号vs交易时段
    getGroupBy: {
      method: 'POST',
      reduce: (state, action) => {
        ...
        if (action.body) {
          tellerandtimel1List = formatGroupByBranchNumAndHour(action.body.data);
        }
        if (status === 'resolved') {
