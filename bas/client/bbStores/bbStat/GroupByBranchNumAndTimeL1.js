...
let branchNumAndTimeL1List = null;

export const {types, actions, rootReducer} = createResource({
  name: 'branchNumAndTimeL1',
  url: `${hostUrl}/cases/:case_id/bbills/branch/group-by-branchnumandtimel1`,
  actions: {
    //B3-机构号vs交易时段
    getGroupBy: {
      method: 'POST',
      reduce: (state, action) => {
        ...
        if (action.body) {
          branchNumAndTimeL1List = formatGroupByBranchNumAndHour(action.body.data);
        }
        if (status === 'resolved') {
